...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-[#0f172a] text-slate-300 flex flex-col transition-all duration-300 shadow-2xl lg:shadow-none border-r border-slate-800 ${
          isMobileOpen ? 'translate-x-0 w-72' : '-translate-x-full lg:translate-x-0'
        } ${isCollapsed ? 'lg:w-20' : 'lg:w-72'}`}
      >
        {/* Sidebar Brand Header */}
        <div className="h-20 px-4 flex items-center justify-between border-b border-slate-800/80 bg-[#0b1329]">
          <div className="flex items-center space-x-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-white p-1.5 flex items-center justify-center flex-shrink-0 shadow-md ring-2 ring-white/10">
              <img
                src="https://res.cloudinary.com/df01whs60/image/upload/v1785656956/Sant_haridas_hospital_logo_page-0001_vu9ssi.jpg"
                alt="Sant Haridas Hospital"
                className="w-full h-full object-contain"
          ...
          <button
            ...
            className="hidden lg:flex w-7 h-7 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 items-center justify-center transition-colors"
        ...
        {/* User Card */}
        <div className={`p-4 border-b border-slate-800/80 bg-[#0d172e] ${isCollapsed && !isMobileOpen ? 'text-center' : ''}`}>
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-teal-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm shadow-md flex-shrink-0">
            ...
            {(!isCollapsed || isMobileOpen) && (
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-teal-500/20 text-teal-300 border border-teal-500/30 capitalize">
                    {currentUser.role}
                  </span>
                </div>
              </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1 custom-scrollbar">
          {navigation.map((item, idx) => {
            ...
            return (
              <div key={item.name}>
                {showSection && (
                  <p className="px-3 pt-3 pb-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    {item.section}
                  </p>
                )}
                <Link
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-teal-400 rounded-r-full shadow-lg shadow-teal-400/50" />
                  )}
                  <Icon
        ...
        <div className="p-3 border-t border-slate-800/80 bg-[#0b1329] space-y-1">
          ...
          <Link
            ...
            className={`flex items-center ${
              isCollapsed && !isMobileOpen ? 'justify-center' : 'space-x-3'
            } px-3 py-2 rounded-xl text-xs font-medium text-slate-400 hover:text-teal-300 hover:bg-slate-800/50 transition-colors`}
            title="View Live Website"
          >
            <ExternalLink className="w-4 h-4 text-teal-400" />
            {(!isCollapsed || isMobileOpen) && <span>View Live Website</span>}
          </Link>
          ...
          <button
            ...
            className={`w-full flex items-center ${
              isCollapsed && !isMobileOpen ? 'justify-center' : 'space-x-3'
            } px-3 py-2 rounded-xl text-xs font-semibold text-rose-300/80 hover:text-rose-200 hover:bg-rose-950/40 transition-colors`}
            title="Sign Out"
          >
